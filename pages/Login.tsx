...
const Login: React.FC = () => {
    const LOGIN_URL_HTTPS = "https://app55.nu.edu.bd/nu-web/applicantLogin.action?degreeName=Professional";
    const LOGIN_URL_HTTP = "http://app55.nu.edu.bd/nu-web/applicantLogin.action?degreeName=Professional";
    const [iframeSrc, setIframeSrc] = useState(() => {
        // Smart initialization based on current protocol
        if (typeof window !== 'undefined' && window.location.protocol === 'http:') {
            return LOGIN_URL_HTTP;
        }
        return LOGIN_URL_HTTPS;
    });
    const [showFallback, setShowFallback] = useState(false);
    const [isLoading, setIsLoading] = useState(true);
    const displayUrl = new URL(LOGIN_URL_HTTP).hostname;
                        ...
                        {showFallback ? (
                            <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-gray-800">
                                ...
                                <p className="text-slate-600 text-sm lg:text-base dark:text-slate-400 mb-6 max-w-md">
                                    The external login page refused to connect. This usually happens due to browser security settings or the server being down.
                                </p>
                                <a
                                    href={LOGIN_URL_HTTP}
                                    target="_blank"
